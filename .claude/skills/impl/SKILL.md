...
- **API 타입 동기화 (백엔드 연동 작업 시)**: 백엔드 spec 변경이 있었으면 **먼저** `pnpm --filter @magampick/api-types gen` 재생성 → `git diff` 확인. API 요청/응답 타입은 손으로 적지 말고 생성 타입 `@magampick/api-types` (`/customer` · `/seller`) 우선 참조, Zod 스키마는 거기에 맞춰 작성 ([`api-client-convention.md §7-1`](../../../docs/api-client-convention.md))
...
| axios interceptor / 에러 정규화 / Zod 응답 검증 / 생성 타입(`@magampick/api-types`) 참조·재생성 | [`api-client-convention.md`](../../../docs/api-client-convention.md) (§7-1) |
