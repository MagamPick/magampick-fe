...
/** 편집 행 공통 클래스 (프로토타입 08-c4 `.profile-edit-row` — 아이콘 없음, py16, min-h56) */
const EDIT_ROW =
...
/**
 * 내 정보 수정 (프로토타입 59-edit-profile). 노션 "소비자 프로필 관리".
 * 닉네임만 실제 수정(시트). 대표 이메일 = 읽기 전용(계정 식별자). 비밀번호·휴대폰·사진 = 비범위 → 준비 중.
 */
          ...
          <>
            ...
            <div className="flex justify-center pb-2 pt-7">
              <button
                ...
                className="relative flex size-24 items-center justify-center rounded-full bg-[linear-gradient(135deg,#FCE6F1_0%,#F9D4E5_100%)] transition active:scale-95"
              >
            ...
            <div className="mx-5 mt-[18px] overflow-hidden rounded-[14px] border border-border bg-card">
              ...
              <button type="button" onClick={() => setSheetOpen(true)} className={EDIT_ROW}>
                <span className={ROW_LABEL}>닉네임</span>
                <span className={ROW_VALUE}>{profile.nickname}</span>
