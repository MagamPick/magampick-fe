 ...
 * 는 건드리지 않는다. `text-disabled #BDBDBD` 는 디자인 토큰이지만 아직 globals.css 에 없어
 * 인라인 — 토큰 추가 시 `text-*` 유틸로 교체.
 *
 * 폭은 콘텐츠(`TabLayout` 의 `max-w-md`)와 동일하게 뷰포트 하단 **중앙 고정** — 데스크탑에서
 * 네비만 화면 끝까지 퍼지지 않게 한다. 모바일(≤448px)에선 좌우 12px(`px-3`) 여백으로 동작 동일.
 * (프로토타입은 `.phone` 안 `absolute` 였으나 실제 PWA 뷰포트에선 `fixed` + 노치 안전영역(`env`)
 *  + 8px, 안전영역 없으면 24px 폴백 = 프로토 수치로 교정.)
 */
