...
// 중앙 정렬 팝업 모달 (약관 보기 등). 바텀 시트는 sheet.tsx 사용.
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "fixed left-1/2 top-1/2 z-50 flex w-[calc(100%-2.5rem)] max-w-[360px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-card shadow-xl",
...
function DialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col gap-2 p-5 pt-3", className)}
      {...props}
    />
  )
}
...
function DialogTitle({
  ...
  return (
    <DialogPrimitive.Title
      ...
      className={cn("text-base font-extrabold tracking-tight text-foreground", className)}
      {...props}
    />
  )
}
