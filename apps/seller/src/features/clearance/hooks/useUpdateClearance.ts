...
/**
 * 떨이 수정 — 활성 떨이의 할인가·남은 수량·픽업 마감 변경.
 * 성공 시 상세 + 떨이 목록 무효화. 남은 수량 0(→품절)은 상품 배지에도 영향 → 상품 목록도 무효화.
 */
export function useUpdateClearance(id: string, storeId: string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (payload: UpdateClearancePayload) => clearanceApi.updateClearance(id, payload),
    onSuccess: () => {
