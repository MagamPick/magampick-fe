...
const fields = { name: '녹차 라떼', category: 'BEVERAGE' as const, price: 5000, onSale: true }

describe('useCreateProduct', () => {
  beforeEach(() => vi.clearAllMocks())

  it('등록 성공 시 storeId 와 함께 호출하고 상품 목록을 무효화한다', async () => {
    const created: Product = { id: 1, storeId: 1, ...fields }
    vi.mocked(productApi.createProduct).mockResolvedValue(created)
    const { queryClient, wrapper } = setup()
    const invalidate = vi.spyOn(queryClient, 'invalidateQueries')
    const { result } = renderHook(() => useCreateProduct(1), { wrapper })

    result.current.mutate(fields)

    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(productApi.createProduct).toHaveBeenCalledWith(1, fields)
