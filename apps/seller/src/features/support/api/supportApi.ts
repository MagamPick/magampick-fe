...
/**
 * 고객센터 API — 노션 「문의하기」(Phase 11). FAQ 조회 + 1:1 문의 제출·내역·답변 조회 (사장).
 * BE 엔드포인트: GET /seller/faqs · GET·POST /seller/inquiries · GET /seller/inquiries/{id}
...
export const supportApi = {
  ...
  async listFaqs(): Promise<Faq[]> {
    const { data } = await apiClient.get('/seller/faqs')
  ...
  async listInquiries(): Promise<Inquiry[]> {
    const { data } = await apiClient.get('/seller/inquiries')
    return z
  ...
  async getInquiry(id: string): Promise<Inquiry> {
    const { data } = await apiClient.get('/seller/inquiries/' + id)
  ...
  async submitInquiry(input: InquiryInput): Promise<Inquiry> {
    ...
    const { data } = await apiClient.post('/seller/inquiries', body)
    return mapInquiry(inquiryResponseSchema.parse(data))
  },
}
