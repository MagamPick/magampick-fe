...
> 프로토타입은 **UI 마크업·스타일의 정확한 스펙 소스**. 동작 로직(바닐라 JS)은 React + TanStack Query + 노션 명세로 새로 짜므로 직접 베끼지 않지만, **시각·구조 수치는 그대로 따른다.** 구현 중 발견하는 "이상한 것"(거의 맞지만 조금씩 어긋난 UI/CSS/네비게이션)은 고치면서 진행.

---

## 구현 충실도 — 눈대중 X, 소스에서 스펙 추출

화면을 프로토타입과 맞출 땐 **눈대중 금지.** 해당 화면의 실제 소스를 읽어 정확한 수치를 뽑은 뒤 구현하고, 끝나면 스크린샷으로 대조한다.

- **위치 찾기**: 아래 화면 인덱스 표에서 화면 → HTML 라인. CSS/JS 파일은 각 앱 `README.md` 매핑.
- **DOM 구조·텍스트·상태·검증 로직**: `scripts/NN-*.js` (예: `12-signup.js` 의 `refreshSignupCta` / `pwScore` / step별 valid).
- **정확한 px·radius·색·상태 스타일**: `styles/*.css` — `03-components.css`(공용 `.field`/`.field__input`/`.btn`/`.chk`), `04-auth.css`(화면별), `01-tokens.css`(토큰).
- ⚠️ **shadcn/라이브러리 기본값이 프로토타입과 다를 수 있다** (예: shadcn `Input` 기본 `h-9`=36px vs 프로토타입 `.field__input` 50px → 인풋 안 suffix 버튼이 튀어나옴). 필요하면 shared 컴포넌트 기본 클래스 자체를 프로토타입 스펙으로 오버라이드. 색은 `globals.css` 토큰이 `01-tokens.css` 와 1:1 이어야 한다.

### 스크린샷 대조 (UI 구현 후 자기검증)

구현 후 **Playwright(headless, viewport 393×852)** 로 화면·단계별 스크린샷을 찍어 프로토타입과 나란히 대조 → 어긋난 부분 교정. **임시 검증용** — 스크립트·스크린샷(`_shot.mjs`, `.shots/` 등)은 **커밋 전 삭제.** (CI 에서 도는 커밋 E2E 테스트(SKILL §5-6)와 별개.)

**dev 서버 포트 — 슬롯 병렬 충돌 방지** (`vite.config` 안 건드리고 런타임 `--port` 로만):

| 위치 | 포트 (충돌 시 폴백) |
|---|---|
| 메인 `magampick-fe` | `5173` |
| `-wt1` | `15173` (`45173`) |
| `-wt2` | `25173` (`55173`) |
| `-wt3` | `35173` (`65173`) |

`pnpm --filter <app> dev -- --port <포트> --strictPort` 로 띄우고(`--strictPort` 라 충돌 시 실패 → 폴백 포트로 재시도), Playwright 스크립트는 그 포트를 타깃. 끝나면 dev 서버 종료 + 임시파일 삭제.
